...
regions = [["nucleus","\t\tselected regions = "+nucleus1],
           ["cytoplasm","\t\tselected regions = "+cytoplasm1],
           ["membrane","\t\tselected regions = "+membrane1],
           ["cell","\t\tselected regions = "+membrane1+cytoplasm1+nucleus1]]
...
for i in energies: #radionuclide spectra
  for j in regions: #source region
    """
    filename = "sph_"+i[0]+"_mdamb468_"+j[0]+".egsinp"
    replace_line(filename, 92, i[1])
    replace_line(filename, 93, i[2])
    replace_line(filename, 81, j[1])

    filename = "sph_"+i[0]+"_mdamb361_"+j[0]+".egsinp"
    replace_line(filename, 92, i[1])
    replace_line(filename, 93, i[2])
    replace_line(filename, 81, j[1])
    """
    filename = "ell_"+str(i).rjust(3, '0')+"_mdamb"+CellModel+"_"+j[0]+"_"+Angle+".egsinp"
    replace_line(filename, 156, "\t\t\tenergy = "+str(0.001*i)) #electron energy
    #replace_line(filename, 157, i[2]) #
    replace_line(filename, 146, j[1]) #source region

#print(nucleus1+cytoplasm1+membrane1)
